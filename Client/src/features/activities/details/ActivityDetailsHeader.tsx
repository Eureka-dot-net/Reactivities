...
export default function ActivityDetailsHeader({ activity }: Props) {
    const { addAttendance, removeAttendance } = useActivities();
    const changeAttendance = (isGoing: boolean) => {
        if (isGoing) {
            addAttendance.mutate(activity.id);
        } else {
            removeAttendance.mutate(activity.id);
        }
    }
    return (
                    ...
                    {activity.isHost ? (
                        <>
                            <StyledButton
                                ...
                                onClick={() => changeAttendance(activity.isCancelled)
                                }
                                disabled={addAttendance.isPending || removeAttendance.isPending}
                            >
                        ...
                        <StyledButton
                            ...
                            onClick={() => changeAttendance(!activity.isGoing)}
                            disabled={addAttendance.isPending || removeAttendance.isPending || activity.isCancelled}
                        >
