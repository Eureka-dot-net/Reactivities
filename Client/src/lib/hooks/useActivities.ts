...
export const useActivities = (id?: string) => {
    ...
    const useAttendanceMutation = (method: 'post' | 'delete') => {
        return useMutation({
            mutationFn: async (id: string) => {
                await agent[method](`/activities/attend/${id}`);
            },
            onMutate: async (activityId: string) => {
                // Cancel any outgoing or stale queries for this specific activity 
                // as we don't want anything interfering with our optimistic update
    ...
    const addAttendance = useAttendanceMutation('post');
